...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Source Code", value: "vendor_code" },
  ...
  methods: {
    ...
    assignUser(){
      this.$http.secured
        .get("/api/v1/warehouse/wms/documents/assign_user", {params: {id: this.user, document_id: this.selected[0].id}})
        .then(response => {
          if ((response) && (response.data)) {
            this.selected[0].assigned_username = response.data.gate_pass.assigned_username
            this.selected[0].assigned_user_id = response.data.gate_pass.assigned_user_id
            this.user = response.data.gate_pass.assigned_user_id
    ...
    viewDocumentDetails(item){
      ...
      this.selected.push(item)
        ...
        .then(response => {
          if ((response) && (response.data.gate_pass_inventories != undefined)) {
    				this.selected_document_data = response.data.gate_pass_inventories;
            this.user =  item.assigned_user_id
    			}
          else if ((response) && (response.data.inventories != undefined)) {
            this.selected_document_data = response.data.inventories
            this.user =  item.assigned_user_id
          }
    			else{
    				this.showAlert('error', "No Data Found");
    ...
    getDocumentData(tab) {
    	if (tab == "IBD"){
    		var document_type = "IBD"
    	}
    	else if(tab == "OBD"){
    		var document_type = "OBD"
    	}
    	else if(tab == "GI"){
    		var document_type = "GI"
    	}
      this.loading = true;
      ...
      this.$http.secured
        .get("/api/v1/warehouse/wms/documents", {params: {page: params['page'], per_page: params['size'], status: status, document_type: document_type}})
        .then(response => {
          if (response.data) {
            this.original_table_data = response.data.gate_passes;
    ...
    searchItem(){
      ...
      if (this.active_tab == "IBD"){
    		var document_type = "IBD"
    	}
    	else if(this.active_tab == "OBD"){
    		var document_type = "OBD"
    	}
    	else if(this.active_tab == "GI"){
    		var document_type = "GI"
    	}
      ...
      if (this.search.length != 0){
        this.$http.secured.post('/api/v1/warehouse/wms/documents/search_documents', formData)
        .then(response => {
          if (response.data.gate_passes){
    ...
    proceedEditPopup(){
      ...
      this.$http.secured
        .get("/api/v1/warehouse/wms/documents/fetch_inbound_record", { params })
        .then(response => {
              if (response.data) {
    ...
    submitProceedPopup(){
      ...
      this.$http.secured
        .post("/api/v1/warehouse/wms/documents/update_inbound_scan_flag?id="+id, { "gate_pass" : gate_pass})
        .then(response => {
          if (response.data) {
