<template>
  <v-card class="overflow-hidden v-card v-sheet theme--light">
    ...
    <div>
      <HeaderComponent :headerTitle="headerText" :router-link="{ name: 'RetunrCreation' }" :router-link-text="'Return / Claim Requests'" :return-condition="true" :component-name="'Warranty Claim Request'" />
    </div>
    <!-- search invoice -->
         ...
         <v-row style="margin-bottom: -20px;" v-if="select_method && select_method.original_code === 'Article ID & Serial Number'"> 
          ...
          <v-row style="margin-bottom: -10px;">
            ...
            <v-col cols="13" sm="3" md="3">
              <v-btn  v-if="this.selectedAddItem.length > 0" class="ma-2 color-change" @click="openDialogSearchInVoice" color="#ffff">+ ADD ANOTHER INVOICE</v-btn> <br>
    ...
    <v-card class="v-card-100 border-top">
      <v-card-text>
        ...
        <v-row class="">
          <v-col cols="12" sm="4" md="4" ></v-col>
          <v-col cols="12" sm="8" md="8" >
            <span class="ma-2">Date Of Purchase</span><br>
            <v-row>
              <v-col cols="12" sm="6" md="6" class="ma-2 time">
                <v-menu ref="menu_14" v-model="menu_14" :close-on-content-click="false" :return-value.sync="purchase_date" transition="scale-transition" offset-y min-width="290px" class="ma-2">
                    <template v-slot:activator="{ on, attrs }">
                      <v-text-field outlined hide-details v-model="purchase_date" label="Date" prepend-inner-icon="event" readonly v-bind="attrs" v-on="on" ></v-text-field>
                    </template>
                  <v-date-picker :max="new Date().toISOString().substr(0, 10)"  v-model="purchase_date" no-title scrollable class="ma-2">
                    <v-spacer></v-spacer>
                    <v-btn text color="primary" @click="menu_14 = false">Cancel</v-btn>
                    <v-btn text color="primary" @click="$refs.menu_14.save(purchase_date)">OK</v-btn>
                  </v-date-picker>
                </v-menu>
              </v-col>
            </v-row>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn small color="primary" style="margin-top: 18px;" :disabled="number > 0 || !item_location ||  !select_method" @click="submitForm">SUBMIT</v-btn>
        </v-card-actions>
      </v-card-text>
    </v-card>
  </v-card>
  ...
  methods: {
    ...
    changeLocation() {
      if (this.item_location) {
        const selectedlocation = this.item_location.original_cod;
        if (selectedlocation === 'Location ID') {
        }
      }
    },
    // select method script 
    ...
    addItem() {
      ...
      if(this.selected_manf.length > 0){
        ...
        this.successmessage = `${this.selected_manf.length} Items are marked for Warranty Claim Request`
      }
    },
