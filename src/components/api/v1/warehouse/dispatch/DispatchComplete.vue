          ...
          <div class="col-lg-6">
            <div class="pp_test_sec">
              <ul class="qc_test_ul4">
                ...
                <li>Destination : <span>{{selected_value[0] ? selected_value[0].warehouse : "NIL"}}</span></li>
              </ul>
            </div>
          </div>
        </div>
              ...
              <div class="card-header" id="headingOne">
                <div class="table_colmw table_bordr">
                  <div class="tbl_colpic1">
                    <h4 class="table_head1">Gate Pass No - <span> {{selected.gatepass_number}}</span></h4>
                  </div>
                  <div class="tbl_colpic2">
                    <h4 class="table_head1"><br></h4>
                  </div>
                  <div class="tbl_colpic3">
                    <h4 class="table_head1">Boxes: <span> {{selected.box_count}} </span></h4>
                  ...
                  <div class="check_tablw">
                    <div class="table_colmw table_bordr table_poup">
                      ...
                      <div class="tbl_p_pack1">
                        <h4 class="table_head1"><span> {{box.box_number}} </span></h4>
                      </div>
                      <div class="tbl_p_pack2">
                        <h4 class="table_head1"><span><br></span></h4>
                      </div>
                      <div class="tbl_p_pack3">
                        <h4 class="table_head1"><span class="pass_test"><br></span></h4>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <div class="pack_colum2" >
          <form @submit.prevent="createConsignment"  novalidate="true" id="consignmentForm" enctype="multipart/form-data">
            <div class="check_table scroll">
              ...
              <div class="model_drop1 pp_frm_w">
                <select class="search_frm1" v-model="newconsignment.logistics_partner_id" :class="{ 'is-invalid': $v.newconsignment.logistics_partner_id }">
  ...
  export default {
    ...
    data () {
      return {
        ...
        file: '',
        isActiveModal: true,
        selected_ids: this.$store.state.dispatch_gate_passes.map(value => value.id),
        newconsignment: {
    ...
    methods: {
      getGatepasses(){
        this.$http.secured.get('/api/v1/warehouse/warehouse_dispatch/get_selected_gate_passes', {params:{ids: this.selected_ids}})
          .then(response => { this.selected_value = response.data.gate_passes })
      ...
      getLogistics(){
        this.$http.secured.get('api/v1/warehouse/warehouse_dispatch/logistics')
          .then(response => { this.logistics = response.data.logistics_partners })
          .catch(error => { this.$toast.error('Error in fetching Logistics')})
      },
      getConsignmentFileType(){
        this.$http.secured.get('/api/v1/warehouse/warehouse_dispatch/consignment_file_types')
          .then(response => { this.consignment_file_types = response.data.lookup_values })
          .catch(error => { this.$toast.error('Error in fetching Consignment File Type') })
      },
      createConsignment(){
        this.showDetails()
        ...
        this.$http.secured.post('/api/v1/warehouse/warehouse_dispatch/create_consignment', formData)
          .then(response => {
            this.isActiveModal = false
            this.$toast.success('Dispatch successfully completed')
            this.$router.push('/api/v1/warehouse/dispatch')
          })
