      ...
      <v-card class="overflow-hidden v-card v-sheet theme--light">
        ...
        <v-card-title>          
          <v-spacer></v-spacer>
            <v-card-actions >
            <v-flex xs12 d-flex>
              <v-btn color="primary" dense class="mr-4 mt-1" to="/api/v1/expArticlesSerialNumberFileUploadList">Exceptional Article Serial Number Mapping File Upload List</v-btn>
            </v-flex>
          </v-card-actions>
        </v-card-title>
        <v-layout column>
  ...
  export default {
    data() {
      return {
        menu: false,
        logDialog: false,
        headerText: "Exceptional Articles Serial Number Mapping List",
        loading: true,
        alert: false,
        search: "",
        alert_type: "success",
        ...
        headers: [
          ...
          { text: "Serial Number Length", value: "serial_number_length" }
        ],
        table_data: []
      };
    },
    ...
    methods: {
      getArticleList() {
        this.loading = true;
        this.$http.secured
        .get("/api/v1/list_expectional_article_serial_number")
        .then((response) => {
          if (response) {
