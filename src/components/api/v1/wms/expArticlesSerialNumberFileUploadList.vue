  ...
  <div>
    ...
    <v-row justify="center">
      <v-dialog v-model="fileUploadPopup" max-width="750px">
        <v-card  class="v-card-100">
          <v-card-title>
            <span class="inc-title">Exceptional Article Serial Number Mapping File Upload</span>
          </v-card-title>
          <v-card-text class="pa-0">
      ...
      <v-card class="overflow-hidden v-card v-sheet theme--light">
        ...
        <v-card-title>          
          <v-spacer></v-spacer>
            <v-card-actions >
            <v-flex xs12 d-flex>
              <v-btn color="primary" dense class="mr-4 mt-1" @click="articleFileUploadPopup()">Exceptional Article Serial Number Mapping File Upload</v-btn>
              <v-btn color="primary" dense class="mr-4 mt-1" to="/api/v1/exceptionalArticlesSerialNumber">Exceptional Article Serial Number Mapping List</v-btn>
            </v-flex>
          </v-card-actions>
        </v-card-title>
        <v-layout column>
  ...
  export default {
    data() {
      return {
        menu: false,
        logDialog: false,
        headerText: "Exceptional Articles Serial Number Maping",
        loading: true,
        alert: false,
        search: "",
        file: "",
    ...
    methods: {
      getFileList() {
        this.loading = true;
        this.$http.secured
        .get("/api/v1/exp_articles_file_upload?file_type=Exceptional Article Serial Number")
        .then((response) => {
          if (response) {
      ...
      importFile () {
        ...
        const params = {master_file: this.file,master_file_type: 'Exceptional Article Serial Number', 
